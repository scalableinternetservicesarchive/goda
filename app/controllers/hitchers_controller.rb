class HitchersController < ApplicationController
  ...
  def index
    @flag = false
    @search = Search.new(Hitcher, params[:search], :per_page => 20) 
#    @hitchers = Hitcher.all
    if params[:search] == nil
        @hitchers = Hitcher.paginate(page: params[:page], per_page: 20) if stale?([Hitcher.paginate(page: params[:page], per_page: 20)])
        @flag = true
    else
        @flag = false
        @hitchers = @search.run
